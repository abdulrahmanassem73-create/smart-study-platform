...
- يعتمد على متغيرات البيئة في Vite:
  - VITE_SUPABASE_URL
...
export async function checkSupabaseConnection() {
  ...
  try {
    // خفيف وآمن: ping على REST عبر select بسيط.
    // لو RLS قافلة بدون session: غالباً هيطلع خطأ 401/permission — ساعتها نعتبر الاتصال موجود لكن محتاج Login.
    const { error } = await sb.from("files").select("id").limit(1);

    if (!error) return { ok: true, error: null as any };

    const msg = String((error as any).message || "");
    // أخطاء متوقعة قبل تسجيل الدخول
    if (
